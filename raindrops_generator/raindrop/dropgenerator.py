...
def generateDrops(imagePath, cfg, listFinalDrops, label_map):
    ...
    for drop in listFinalDrops:
        ...
        if (drop.shape == 1):
            ...
            cv2.ellipse(
                mask,
                drop.center,
                (drop.radius, int(1.3*math.sqrt(3) * drop.radius)),
                0,
                180,
                360,
                (255, 255, 255),
    ...
    for drop in listFinalDrops:
        if (drop.shape == 2):
            ...
            xys = [
                (drop.radius * C[0] - 2*drop.radius + drop.center[0], drop.radius * C[1] - 3*drop.radius + drop.center[1]),
                (drop.radius * B[0] - 2*drop.radius + drop.center[0], drop.radius * B[1] - 3*drop.radius + drop.center[1]),
                (drop.radius * D[0] - 2*drop.radius + drop.center[0], drop.radius * D[1] - 3*drop.radius + drop.center[1])
            ]
            ...
            xys = [
                (drop.radius * C[0] - 2*drop.radius + drop.center[0], drop.radius * C[1] - 3*drop.radius + drop.center[1]),
                (drop.radius * A[0] - 2*drop.radius + drop.center[0], drop.radius * A[1] - 3*drop.radius + drop.center[1]),
                (drop.radius * D[0] - 2*drop.radius + drop.center[0], drop.radius * D[1] - 3*drop.radius + drop.center[1])
            ]
            bezier = make_bezier(xys)
    ...
    for drop in listFinalDrops:
        ...
        #     ROIL = imgw - 4*radius
        #     ROIR = imgw
        tmp_bg = np.zeros((ROID-ROIU, ROIR-ROIL, 3), dtype=np.uint8)
        if ROIU < 0:
            tmp_roiu = -ROIU
            bg_roiu = 0
        else:
        ...
        if ROIR > imgw:
            ...
            tmp_roir = tmp_bg.shape[1]
            bg_roir = ROIR

        bg_h = bg_roid-bg_roiu
        bg_w = bg_roir-bg_roil
        tmp_h = tmp_roid-tmp_roiu
        tmp_w = tmp_roir-tmp_roil
        if bg_h != tmp_h or bg_w != tmp_w:
            print(ROIU, ROID, ROIL, ROIR)
            print(bg_roiu, bg_roid, bg_roil, bg_roir)
            print(tmp_roiu, tmp_roid, tmp_roil, tmp_roir)
        tmp_bg[tmp_roiu:tmp_roid, tmp_roil:tmp_roir] = bg_img[bg_roiu:bg_roid, bg_roil:bg_roir]
        drop.updateTexture(tmp_bg)
        tmp_alpha_map = np.zeros((ROID-ROIU, ROIR-ROIL), dtype=np.float64)
        tmp_alpha_map[tmp_roiu:tmp_roid, tmp_roil:tmp_roir] = alpha_map[bg_roiu:bg_roid, bg_roil:bg_roir]
